...
void player_update(Player *player, int *level) {
  ...
  if (player->is_grounded)
    player->coyote_time_left = player->coyote_time;
  else if (player->coyote_time_left > 0)
    player->coyote_time_left -= FIXED_DT;

  if (jump_pressed)
    player->jump_buffer_left = player->jump_buffer;
  else if (player->jump_buffer_left > 0)
    player->jump_buffer_left -= FIXED_DT;

  if (shift_pressed)
    player->shift_buffer_left = player->shift_buffer;
  else if (player->shift_buffer_left > 0)
    player->shift_buffer_left -= FIXED_DT;
  ...
  if (shift_held) {
    ...
    if ((player->jump_buffer_left > 0 && player->is_grounded) || (jump_pressed && player->coyote_time_left > 0)) {
    ...
    if (move_dir.y < 0 && !jump_held && player->vel.y < player->min_jump_speed) {
    ...
    Vector2 acc = (Vector2){input_dir.x * player->speed * FIXED_DT, fall_speed * FIXED_DT};
