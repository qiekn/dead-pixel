...
void setup_linked_list(Boid *boids, int *link_heads) {
  for (int i = 0; i < NUM_BOIDS; i++) {
    Vector2 position = (Vector2){GetRandomValue(2, MAP_WIDTH - 2) * CELL_SIZE, GetRandomValue(2, MAP_HEIGHT - 2) * CELL_SIZE};
    Vector2 direction = Vector2Normalize((Vector2){GetRandomValue(-64, 64), GetRandomValue(-64, 64)});
...
void update_boids(Player *player, Boid *boids, int *link_heads, Vector2 *average_positions, Vector2 *average_directions,
  ...
  for (int i = 0; i < GRID_CELLS; i++) {
    ...
    while (current != -1) {
      ...
      if (remaining_x >= GRID_HALF_SIZE)
        horizontal++;
      else if (remaining_x < GRID_HALF_SIZE)
        horizontal--;
      if (remaining_y >= GRID_HALF_SIZE)
        vertical++;
      else if (remaining_y < GRID_HALF_SIZE)
        vertical--;
      ...
      for (int y = 0; y != vertical * 2; y += vertical) {
        ...
        for (int x = 0; x != horizontal * 2; x += horizontal) {
          ...
          while ((inside != -1)) {
            ...
            average_separation = Vector2Subtract(average_separation,
                                                 Vector2Scale(Vector2Subtract(boids[current].position, boids[inside].position), 1.0f / distance_sqr));
            separation_count++;
