...
void restart_sequence(Player *player, char restart_message_target[RESTART_MESSAGE_LENGTH], char restart_message_live[RESTART_MESSAGE_LENGTH],
                      int *char_index, int *delay_left);
...
int main(void) {
  ...
  player.aabb = (Rectangle){100, 500, MIN_PLAYER_SIZE, MIN_PLAYER_SIZE};
  ...
  while (!WindowShouldClose()) {
    ...
    switch (current_state) {
      case STATE_GAME:
        ...
        Vector2 player_centre = (Vector2){player.aabb.x + player.aabb.width / 2, player.aabb.y + player.aabb.height / 2};

        Vector2 level_offset = (Vector2){(int)(player_centre.x / WINDOW_WIDTH), (int)(player_centre.y / WINDOW_HEIGHT)};

        Vector2 render_offset = (Vector2){level_offset.x * LEVEL_WIDTH, level_offset.y * LEVEL_HEIGHT};

        Vector2 camera_offset = (Vector2){level_offset.x * WINDOW_WIDTH, level_offset.y * WINDOW_HEIGHT};
        ...
        // WIN
        if (player.is_maxxed_out && player.aabb.width >= MAX_PLAYER_WIDTH - 10 && player.aabb.height >= MAX_PLAYER_HEIGHT - 10 &&
            !player.is_shifting) {
        ...
        for (int i = 0; i < NUM_BOIDS; i++) {
          ...
          if (boids[i].position.x < render_offset.x * CELL_SIZE || boids[i].position.x > render_offset.x * CELL_SIZE + WINDOW_WIDTH ||
              boids[i].position.y < render_offset.y * CELL_SIZE || boids[i].position.y > render_offset.y * CELL_SIZE + WINDOW_HEIGHT)
            continue;
          ...
          DrawTriangleLines(Vector2Add(boids[i].position, Vector2Scale(boids[i].direction, AVOID_DISTANCE)),
                            (Vector2){boids[i].position.x - BOID_SIZE, boids[i].position.y + BOID_SIZE},
                            (Vector2){boids[i].position.x + BOID_SIZE, boids[i].position.y + BOID_SIZE}, BOID_COLOUR);
        }
        EndMode2D();
        ...
        for (int y = 0; y < LEVEL_HEIGHT; y++) {
          for (int x = 0; x < LEVEL_WIDTH; x++) {
            ...
            if (cell_type == VIRUS) {
              ...
              Rectangle cell_rect = {x * CELL_SIZE, y * CELL_SIZE, CELL_SIZE, CELL_SIZE};
        ...
        if ((player.time_remaining < TIME_REMAINING_GLITCH && !player.is_maxxed_out) ||
            (player.time_remaining > player.max_time - FPS / 2 && !player.is_maxxed_out)) {
      ...
      case STATE_UPGRADE:
        ...
        if (!player.is_maxxed_out) {
          sprintf(upgrades_text, "WIDTH: %d\t\t[%d/%d]\n\n\nHEIGHT: %d\t\t[%d/%d]\n\n\nTIME: %d\t\t[%d/%d]", (int)player.max_width, upgrade_level[0],
                  UPGRADE_LEVELS, (int)player.max_height, upgrade_level[1], UPGRADE_LEVELS, player.max_time, upgrade_level[2], UPGRADE_LEVELS);
        } else {
        ...
        DrawModelEx(cubeModel, (Vector3){0, 0, 0}, (Vector3){0.2f, 0.8f, 0.1f}, time * 100.0f,
                    (Vector3){player.max_width / MAX_PLAYER_WIDTH * 10, player.max_height / MAX_PLAYER_HEIGHT * 10,
                              player.max_height / MAX_PLAYER_HEIGHT * 10},
...
void restart_sequence(Player *player, char restart_message_target[RESTART_MESSAGE_LENGTH], char restart_message_live[RESTART_MESSAGE_LENGTH],
                      int *char_index, int *delay_left) {
  sprintf(restart_message_target,
          "SEGMENTATION FAULT (Core Dumped)\nRestarting, please wait.../DEAD-PIXEL\nFreeing unused kernel memory\n\n-\n\nINIT: version 1.0.0 "
          "booting\n\n[ OK ] Found save file\n     * MAXIMUM_WIDTH = %d\n     * MAXIMUM_HEIGHT = %d\n     * TIME = %d\n     * BUGS COLLECTED = "
          "%d\n\n-\n\nRestart Successful!\n\n---\n\n\n\n\n\n",
          player->max_width == MAX_PLAYER_WIDTH ? 999999 : (int)player->max_width,
          player->max_height == MAX_PLAYER_HEIGHT ? 999999 : (int)player->max_height, player->is_maxxed_out ? 999999 : player->max_time,
...
void reset_game(Player *player, Boid *boids, int *link_heads) {
  player->aabb = (Rectangle){144, 500, MIN_PLAYER_SIZE, MIN_PLAYER_SIZE};
  player->vel = Vector2Zero();
  player->last_grow_direction = Vector2Zero();
  player->coyote_time_left = 0.0f;
  player->jump_buffer_left = 0.0f;
